...
def get_per_sample_metrics_from_predictions(y_preds: np.ndarray, targets: np.ndarray):
    def _update(metrics, precision, recall, f1, fp, tp, fn, iou, accuracy):
    ...
    for pred, gt in zip(preds, _batched_targets(targets)):
        ...
        if gt.shape[0] == 0 and pred.shape[0] == 0:
            _update(metrics, np.nan, np.nan, 0, 0, 0, 0, 1, 1)
            continue
        if pred.shape[0] == 0:
            _update(metrics, np.nan, 0, 0, 0, 0, gt.shape[0], 0, 0)
            continue
        if gt.shape[0] == 0:
        ...
        _update(metrics, float(p), float(r), float(f1), int(fp), int(tp), int(fn), float(iou), float(acc))
    return metrics
...
def get_per_sample_metrics(boxes_xyxy: np.ndarray, scores_per_class: np.ndarray, targets: np.ndarray):
...
@tensorleap_custom_metric("Confusion Matrix", direction=MetricDirection.Upward)
def confusion_matrix_metric(boxes_xyxy: np.ndarray, scores_per_class: np.ndarray, targets: np.ndarray):
    y_preds = format_predictions(boxes_xyxy, scores_per_class)
